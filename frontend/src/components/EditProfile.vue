...
const userInitial = computed(() =>
  userInfo.value.name ? userInfo.value.name.charAt(0).toUpperCase() : "U",
);
  ...
  try {
    ...
    if (response.data && response.data.status === "success") {
      ...
      if (response.data.data.fullName)
        storedUser.name = response.data.data.fullName;
      if (response.data.data.profile_photo)
        storedUser.profile_photo = response.data.data.profile_photo;
    ...
    <div class="content-area">
      <div class="overlap-card">
        <div class="editable-avatar-section">
          ...
          <p class="edit-photo-tip" @click="triggerFileInput">
